...
| `blueprints/blueprint.json` | WordPress Playground preview blueprint — the wp.org **Preview** button boots the plugin into the "Project Asteria" demo (26 posts + 5 pages, graph pre-built, landing on the Graph Explorer). Generated by `bin/generate-content-graph-blueprint.php`; the standalone variant (`blueprints/demo.json`) lives in the plugin's dev-only `blueprints/` folder. |
